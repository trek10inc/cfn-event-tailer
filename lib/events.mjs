...
function logStackEvent(stackEvent) {
  ...
  if (Number.isNaN(numberOfTerminalColumns)) numberOfTerminalColumns = 120;
  numberOfTerminalColumns = 120;
  const columnWidths = [
    (numberOfTerminalColumns - 12) / 5,
    12,
    (numberOfTerminalColumns - 12) / 5,
    (numberOfTerminalColumns - 12) / 5,
    (numberOfTerminalColumns - 12) / 5,
  ...
  const timestamp = `${timeDate.toLocaleTimeString()}`;
  const logLines = [
